...
**Related jobs**. Помимо основной джобы есть буквально соседние джобы, которые либо сонаправлены, либо не сонаправлены друг с другом. Может получиться так что решаю одну задачу, мы убьем другие задачи тем самым сделав только хуже. Например джоба пользователя -> донести до всех важное, поэтому давайте чтобы доставляемость была выше, мы будем пинговать и стучать в пользователя с помощью @all. Тем временем задача пользователя по другую сторону -> закрыть важные задачи (это и есть related job). Его задача не будет выполняться если будут часто пинговать т.к он будет отвлекаться. Он замьютит чат, поэтому задача первого пользователя не закроется, доставляемость станет хуже. 
