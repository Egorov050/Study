...
`dat.get('items'`) и `dat['items']` похожи, но не совсем одинаковы:

`dat['items']` обращается напрямую к ключу 'items'. Если этого ключа нет в словаре dat, возникает ошибка KeyError, которая прерывает выполнение программы.

`dat.get('items')`  выполняет ту же задачу, но безопаснее. Если ключ `items` отсутствует в словаре dat, .get() вернёт None (или значение по умолчанию, если оно указано вторым аргументом), что позволяет избежать ошибки.
<h3>Params</h3>
...
- `page` : 1 — указывает, какую страницу результатов нужно вернуть. Здесь указана страница 1, то есть запрос вернет первые 100 результатов (с учетом `per_page` : 100).
Страницы результатов” в контексте API — это способ разбить большое количество данных на более управляемые части. Когда API возвращает результаты, например, для поиска репозиториев, пользователей или сообщений, он может ограничить количество элементов, чтобы запрос не возвращал сразу слишком много данных.
...
Чтобы протестить , что видит сервер , когда мы обращаемся к нему мы можем глянуть это все на след сайте : https://httpbin.org/#/Request_inspection/get_headers
